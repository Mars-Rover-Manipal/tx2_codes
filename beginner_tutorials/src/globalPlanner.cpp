...
void globalPlanner::distance()
{
   ...
   // //Formula for distance calculation in metres using boost
//	double diff_long_deg = diff_long * M_PI / 180.0;
//	double diff_lat_deg = diff_lat * M_PI / 180.0;

//	double goal_long_deg = goal_long_ * M_PI / 180.0;
//	double goal_lat_deg = goal_lat_ * M_PI / 180.0;
	
//	double a = pow(sin(diff_lat_deg / 2), 2) + pow(sin(diff_long_deg / 2), 2) * cos(goal_lat_deg) * cos(sensor_lat_deg); 
//	error_distance_ = 2 * EARTH_RADIUS * asin(sqrt(a));
