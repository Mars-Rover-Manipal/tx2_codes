...
void arucoDetection::imageCallback(const sensor_msgs::ImageConstPtr & msg) {
    ...
    //cv::imshow("Display window",camera_image->image);
    ROS_INFO_STREAM("Got Image");
    //std::cout<<"Image Recieved";
    //camera_image->image = convert_image_encodings(camera_image);
    arucoDetection::detect_aruco();
    //ROS_INFO_STREAM("Marker Detection Ran Successfully");
    //std::cout<<"Marker Detected";
    estimate_pose();
    //ROS_INFO_STREAM("Pose Estimated");
        ...
        if (cv::determinant(rotMat) > 0.99 && cv::determinant(rotMat) < 1.01) {
            ...
            if (marker_ids_detected.size() > 0) {
                ...
                nh.setParam("/marker_ids_detected",marker_ids_detected[0]);
            		...
            		nh.setParam("/marker_ids_detected", 256);
            }
                        
          if (times_detected > 5) {
          	nh.setParam("/arucoDetected", 1);
